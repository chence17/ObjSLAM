 ...
 * @LastEditTime: 2021-04-05 13:34:08
 * @LastEditors: Antonio Chan
 * @Description: Modified from ORB-SLAM2
 * @FilePath: /Localization/src/System.cc
 */
...
System::System(const string &strVocFile, const string &strSettingsFile,
  ...
  // STEP: 初始化词袋模型
  // 词袋是在做闭环检测和重定位时候做检索用的，从历史关键帧中检索出和当前帧最相似的一帧，以进行位姿匹配。
  // Load ORB Vocabulary
  ...
  // STEP: 初始化关键帧数据集
  // 关键帧数据集模块除了加载词袋数据数据以外，还实现了闭环检测和重定位检测。
  ...
  // STEP: 根据bUseViewer判断是否初始化显示界面并打开可视化线程
