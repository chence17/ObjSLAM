...
class ORBmatcher {
 public:
   ...
   * @param CurrentFrame: 当前帧
   * @param pKF: 关键帧
   * @param sAlreadyFound: 已经寻找得到的地图点
   * @param th: 窗口大小的阈值
   * @param ORBdist: 描述子最小距离阈值
   * @return (int) 匹配到的点的数目
   */
   ...
   * @param Scw: 相似变换
   * @param vpPoints: 空间点
   * @param vpMatched: 已经得到的空间点和关键帧上点的匹配关系
   * @param th: 搜索窗口的阈值
   * @return (int) 匹配的特征点数目
   */
   ...
   * @param F1: 初始化参考帧
   * @param F2: 当前帧
   * @param vbPrevMatched: 本来存储的是参考帧的所有特征点坐标,
   * 该函数更新为匹配好的当前帧的特征点坐标
   * @param vnMatches12: 保存参考帧F1中特征点是否匹配上,
   * index保存是F1对应特征点索引, 值保存的是匹配好的F2特征点索引
   * @param windowSize: 搜索窗口
  ...
  /**
   * @brief
   * 通过Sim3变换, 确定pKF1的特征点在pKF2中的大致区域, 同理,
   * 确定pKF2的特征点在pKF1中的大致区域
   * @note
   ...
   * a Sim3 [s12*R12|t12] NOTE In the stereo and RGB-D case, s12=1
   * @param pKF1: 关键帧1
   * @param pKF2: 关键帧2
   * @param vpMatches12: 两帧特征点的匹配关系
   * @param s12: 缩放因子,SIM3中的吧
   * @param R12: #TODO
   * @param t12: #TODO
   * @param th: 搜索窗口阈值
   * @return (int) 匹配到的点的个数
  ...
  /**
   * @brief 将地图点投影到关键帧中进行匹配和融合;
   * 并且地图点的替换可以在这个函数中进行
   * @note Project MapPoints into KeyFrame and search for duplicated MapPoints.
   * @param pKF: 关键帧
  ...
  /**
   * @brief 将地图点投影到关键帧中进行,
   * 但是由于种种原因,地图点还不能够在这个函数中完成替换操作
   * @note Project MapPoints into KeyFrame using a given Sim3 and search for
   * duplicated MapPoints.
   * @param pKF: 关键帧
   * @param Scw: 仿射变换
   * @param vpPoints: 给出的地图点
   * @param th: 搜索窗口阈值
   * @param vpReplacePoint: 需要替换掉的地图点,键值对
   * @return int                  融合的地图点个数
   */
 ...
 public:
  ...
  // (static const int) 判断描述子距离时比较低的那个阈值,
  // 主要用于基于词袋模型加速的匹配过程,
  // 可能是感觉使用词袋模型的时候对匹配的效果要更加严格一些
  static const int TH_LOW;

  // (static const int) 判断描述子距离时比较高的那个阈值,
  // 用于计算投影后能够匹配上的特征点的数目；
  // 如果匹配的函数中没有提供阈值的话, 默认就使用这个阈值
  static const int TH_HIGH;

  // (static const int) 判断特征点旋转用直方图的长度
 ...
 protected:
   ...
   * @note
   * @param histo: 匹配特征点对旋转方向差直方图
   * @param L: 直方图尺寸
   * @param ind1: bin值第一大对应的索引
   * @param ind2: bin值第二大对应的索引
   * @param ind3: bin值第三大对应的索引
   * @return None
   */
