...
class Frame {
 public:
   ...
   * @note 复制构造函数, mLastFrame = Frame(mCurrentFrame)
   * 如果不是自定以拷贝函数的话,
   * 系统自动生成的拷贝函数对于所有涉及分配内存的操作都将是浅拷贝,
  ...
  /**
   * @brief Constructor for stereo cameras. 为双目相机准备的构造函数
   * @note
   * @param imLeft: 左目图像
   * @param imRight: 右目图像
   * @param timeStamp: 时间戳
   * @param extractorLeft: 左目图像特征点提取器句柄
   * @param extractorRight: 右目图像特征点提取器句柄
   * @param voc: ORB字典句柄
   * @param K: 相机内参矩阵
  ...
  /**
   * @brief Constructor for RGB-D cameras. 为RGBD相机准备的帧构造函数
   * @note
   * @param imGray: 对RGB图像灰度化之后得到的灰度图像
   * @param imDepth: 深度图像
   * @param timeStamp: 时间戳
   * @param extractor: 特征点提取器句柄
   * @param voc: ORB特征点词典的句柄
   * @param K: 相机的内参数矩阵
   * @param distCoef: 相机的去畸变参数
   * @param bf: baseline*bf
   * @param thDepth: 远点和近点的深度区分阈值
   */
  ...
  /**
   * @brief Extract ORB on the image. 提取图像的ORB特征
   * @note 0 for left image and 1 for right image.
   * 提取的关键点存放在mvKeys, 描述子存放在mDescriptors
   * @param flag: 标记是左图还是右图.0：左图  1：右图
   * @param im: 等待提取特征点的图像
   * @return None
   */
  ...
  /**
   * @brief 计算词袋模型
   * @note Compute Bag of Words representation.
   * 存放在mBowVec中, 计算词包 mBowVec 和 mFeatVec,
   * 其中 mFeatVec 记录了属于第i个node(在第4层)的ni个描述子
   * CreateInitialMapMonocular() TrackReferenceKeyFrame() Relocalization()
   * @return None
   */
  void ComputeBoW();
   ...
   * @note 用 Tcw 更新 mTcw 以及类中存储的一系列位姿, 用Tcw更新mTcw
   * @param Tcw: 从世界坐标系到当前帧相机位姿的变换矩阵
   * @return None
   */
   ...
   * @note 其实就是根据Tcw计算mRcw、mtcw和mRwc、mOw.
   * @return None
   */
   ...
   * @note
   * mRwc存储的是从当前相机坐标系到世界坐标系所进行的旋转,
   * 而我们一般用的旋转则说的是从世界坐标系到当前相机坐标系的旋转
   * @return (cv::Mat) 返回从当前帧坐标系到世界坐标系的旋转
   */
   ...
   * Step 1 获得这个地图点的世界坐标
   * Step 2
   * 关卡一：检查这个地图点在当前帧的相机坐标系下, 是否有正的深度.如果是负的,
   * 表示出错, 返回false Step 3 关卡二：将MapPoint投影到当前帧的像素坐标(u,v),
   ...
   * 记录计算得到的一些参数
   * @param pMP: 当前地图点
   * @param viewingCosLimit: 夹角余弦, 用于限制地图点和光心连线和法线的夹角
   * @return (bool) true代表地图点合格, 且在视野内;false代表地图点不合格, 抛弃
   */
  bool isInFrustum(MapPoint *pMP, float viewingCosLimit);

  /**
   * @brief
   * 计算某个特征点所在网格的网格坐标, 如果找到特征点所在的网格坐标,
   * 记录在nGridPosX,nGridPosY里, 返回true, 没找到返回false
   * @note
   * @param kp: 给定的特征点
   * @param posX: 特征点所在网格坐标的横坐标
   * @param posY: 特征点所在网格坐标的纵坐标
   * @return (bool) true代表如果找到特征点所在的网格坐标, 返回true;
   * false代表没找到返回false
   */
  bool PosInGrid(const cv::KeyPoint &kp, int &posX, int &posY);

  /**
   * @brief 找到在以x,y为中心,半径为r的圆形内且金字塔层级在[minLevel,
   * maxLevel]的特征点
   * @note
   * @param x: 特征点坐标x
   * @param y: 特征点坐标y
   ...
   *
   * 这里所说的SAD是一种双目立体视觉匹配算法,
   * 可参考[https://blog.csdn.net/u012507022/article/details/51446891]
   *  最后对所有SAD的值进行排序,
   * 剔除SAD值较大的匹配对, 然后利用抛物线拟合得到亚像素精度的匹配
   * 这里所谓的亚像素精度, 就是使用这个拟合得到一个小于一个单位像素的修正量,
   * 这样可以取得更好的估计结果, 计算出来的点的深度也就越准确 匹配成功后会更新
   * Frame::mvuRight (ur) 和 Frame::mvDepth (Z)
   * @return None
   */
   ...
   * @note
   * Associate a "right" coordinate to a keypoint
   * if there is valid depth in the depthmap.
   * 博客[https://www.cnblogs.com/panda1/p/7001052.html]中说,
   * 这个是将地图点和其深度对应起来. 是不是可以这样理解,
   * 为了能够将特征点反投影到三维空间中得到其在相机坐标系以及在世界坐标系下的坐标,
   * 我们需要获得它 在当前相机下的深度.对于双目相机,
   * 我们是通过计算左侧图像中特征点在右图中的坐标,
   * 然后计算其深度；对于RGBD图像我们可以直接 从深度图像上获得特征点的深度,
   * 不过为了处理上的一致这里使用这个深度计算了彩色图像(左图)中的特征点在假想的“右图”中的
   * 坐标.这就是这个函数的工作.
   * @param imDepth: 深度图像
   * @return None
   */
   ...
   * @note
   * Backprojects a keypoint (if stereo/depth info available)
 ...
 public:
   ...
   *   注意这里的相机内参数其实都是类的静态成员变量；
   *   此外相机的内参数矩阵和矫正参数矩阵却是普通的成员变量,
   *   #NOTE 这样是否有些浪费内存空间？
   *
   *=============================================**/
   ...
   *   redundant as images must be rectified. In the RGB-D case,
   *   RGB images can be distorted.
   *   mvKeys:原始左图像提取出的特征点(未校正)
   *   mvKeysRight:原始右图像提取出的特征点(未校正)
   *   mvKeysUn:校正mvKeys后的特征点, 对于双目摄像头,
   *   一般得到的图像都是校正好的, 再校正一次有点多余.
   *
   *=============================================**/

  // (std::vector<cv::KeyPoint>) #TODO
  //原始左图像提取出的特征点(未校正)
  ...
  /**==============================================
   * *                   INFO
   *   之所以对于双目摄像头只保存左图像矫正后的特征点,
   *   是因为对于双目摄像头,一般得到的图像都是矫正好的,
   *   这里再矫正一次有些多余.
   *   校正操作是在帧的构造函数中进行的.
  ...
  // m-member v-vector
  // u-指代横坐标,因为最后这个坐标是通过各种拟合方法逼近出来的,
  // 所以使用float存储
  ...
  // (cv::Mat) #TODO
  // Camera pose.
  // 相机姿态, 世界坐标系到相机坐标坐标系的变换矩阵, 是我们常规理解中的相机位姿.
  cv::Mat mTcw;
  ...
  // (float) #TODO
  // 图像金字塔的尺度因子的对数值, 用于仿照特征点尺度预测地图点的尺度
  ...
  // 所以这里设置了这个变量.
  // 如果这个标志被置位, 说明再下一帧的帧构造函数中要进行这个“特殊的初始化操作”,
  // 如果没有被置位则不用.
